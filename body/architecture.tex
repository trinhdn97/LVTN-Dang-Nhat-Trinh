...
\textbf{Swagger} là một phần mềm mã nguồn mở, chuyên dùng để thiết kế, xây dựng, sử dụng và làm tài liệu cho các ứng dụng web RESTful. Chúng tôi sử dụng thư viện Swagger UI Express \parencite{swagger-ui-express-npm} trên khung thức Node.js để hỗ trợ làm tài liệu cho các \acrshort{api} ở backend và gọi thử các \acrshort{api} đó trong quá trình hiện thực \acrshort{ui} của ứng dụng.\par
